...
loadStorefrontData()
    .then(({ products: items, categories: categoryRecords }) => {
        ...
        categoryRecords.forEach(category => {
            categoryMeta.set(category.name, category);
        });
        ...
        function renderCategoryCards() {
            categoryCards.innerHTML = categories.map(category => {
                ...
                const categoryInfo = categoryMeta.get(category);
                const image = categoryInfo?.image || "https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?auto=format&fit=crop&w=1200&q=80";
                return `
                    <button class="category-card" type="button" data-category-card="${escapeHtml(category)}" style="background-image: linear-gradient(180deg, rgba(33, 31, 29, 0.08), rgba(33, 31, 29, 0.68)), url('${escapeHtml(image)}');">
                        ...
                        <span class="category-card__text">${escapeHtml(categoryInfo?.description || "Открыть категорию")}</span>
                    </button>
                `;
            }).join("");
